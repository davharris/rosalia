...
$\alpha$ and $\beta$ using numerical optimization. Fully-observed networks like the 
ones considered here have unimodal likelihood surfaces [@murphy_machine_2012], so 
these estimates will represent the global optimum. I used the rosalia package 
[@harris_rosalia_2015] for the R programming language [@r_core_team_r_2015] to 
calculate $p(\vec{y}; \alpha, \beta)$ and its gradients [@murphy_machine_2012]; 
the package passes these functions to the "BFGS" method in R's general-purpose 
optimizer, which then estimates the Markov network parameters.
