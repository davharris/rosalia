...
assemblages that these relationships produce. In this section, I show how a set
of conditional relationships can be used to determine the kinds of species
...
marginal association rates can differ from this expectation. Predicting the
marginal relationships between species from their conditional interactions
entails summing over the different possible assemblages (Figure 2B), and the
number of possible assemblages can be intractably large, though some methods
beyond the scope of this paper can be employed to keep the calculations feasible
[@lee_learning_2012; @salakhutdinov_learning_2008]. Alternatively, as noted
below, some common linear and generalized linear methods can also be used as
computationally efficient approximations to the full network
...
prior should be small: the 95% credible interval on $\beta$ only requires that
one species' effect on the odds of observing a different species to be less than
a factor of 1500 (which is not much of a constraint). The logistic distribution
was chosen because it is convex and has a similar shape to the Laplace
distribution used in LASSO regularization (especially in the tails), but unlike
the Laplace distribution it is differentiable everywhere and does not force any
estimates to be exactly zero.

I also evaluated five alternative methods from the existing literature (plus a
novel variant of the fifth method) to calibrate the performance of the Markov
network as a method for estimating species interactions. The first two
alternative interaction measures were the sample correlations and the partial
correlations between each pair of species' data vectors on the landscape
[@albrecht_spatial_2001; @faisal_inferring_2010]. Because partial correlations
are undefined for landscapes with perfectly-correlated species pairs, I used a
regularized estimate based on James-Stein shrinkage, as implemented in the
corpcor package’s `pcor.shrink` function with the default settings
[@schafer_corpcor_2014]. In the context of non-Gaussian data, the partial
...
common for estimating relationships among genes and gene products [@REF].
...
using the other species on the landscape as predictors. [[Observational
...
BayesComm package [@golding_bayescomm_2015] to evaluate the claim from recent
papers that the correlation coefficients estimated by "joint species
distribution models" provide an accurate assessment of species’ pairwise
interactions [@pollock_understanding_2014; see also @harris_generating_2015].  I
used both the average correlation between each pair of species’ latent
variables across 1000 Monte Carlo samples, as well as the average *partial*
correlation, as BayesComm's two estimators of species interactions.
...
interaction values across all the simulated data sets. These squared errors
determined the proportion of variance explained for different combinations of
method and landscape size (compared with a baseline model that assumed all
...
however, there was a consistent ordering, with the four methods that controlled
for indirect effects of species interactions performing the best: the Markov
network explained the largest portion of the variance (34.6% overall), followed
by the generalized linear models (29.9%), partial correlations from the raw
presence-absence data (27.5%), and partial correlations from BayesComm, the
joint species distribution model (26.1%).  The benefit of choosing the full
Markov network over the other three methods was largest on the smaller
landscapes, which are also the ones that are most representative of typical
analyses in this field [[ref]].
...
based on the raw data performed the best (18.6%), followed by the null model
(15.4%) and BayesComm's correlation matrix (11.6%). Although these last three
methods had different $R^2$ values, there was a close mapping between
their estimates (after controlling for the size of the simulated landscapes;
Figure 3b).  This suggests that the correlation matrices from joint species
distribution models and especially the effect sizes from null models only
contain noisy versions of the information that could be obtained more easily by
...
information to provide a single number describing each species pair. To estimate
asymmetric interactions, such as commensalism or predation, ecologists would
need other kinds of data, as from time series, behavioral observations,
manipulative experiments, or natural history. These other sources of information
could also be used to augment the likelihood function with an informative prior
distribution, which could lead to better results on some real data sets than
...
Markov networks can be fit with a mix of discrete and continuous variables,
and likely states of the interaction network can be modeled as a function of
the local abiotic environment [@lee_learning_2012]. There are even methods
[@whittam_species_1981; @tjelmeland_markov_1998] that would allow one species
to affect the sign or strength of the relationship between two other species,
tipping the balance between facilitation and exploitation
[@bruno_inclusion_2003].
...
particular null model must be interpreted with care
[@roughgarden_competition_1983]. In complex ecological networks (and even in
small networks with three species), it may simply not be possible to implicate
...
Estimating pairwise coefficients directly seems like a much more promising
approach: to the extent that the models' relative performance on real data sets
is similar to the range of results shown in Figure 3a, scientists in this field
could often double their explanatory power by switching from null models to
Markov networks (or increase it by nearly as much with linear or generalized
linear approximations). Regardless of the specific methods ecologists ultimately
choose, the most important consideration is clearly that we need to control for
indirect effects and estimate conditional relationships between species, rather
than marginal ones.
...
Golding, N. J. Gotelli, C. F. Dormann, and two anonymous reviewers also provided
useful feedback.
...
independence.  **B.** Relative probabilities of all four possible
presence-absence combinations for Species 1 and Species 2. The exponent includes
...
probabilities, we can find the expected frequencies of all possible
co-occurrence patterns between the two species of interest. **D.** If $\beta$
...
method across varying numbers of sampled locations.
