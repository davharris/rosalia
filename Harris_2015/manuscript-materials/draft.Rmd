\chead{\textit{Species interactions in Markov networks}}
...
[@r_core_team_r_2015] to optimize the Markov network parameters.
...
[@schmidt_modeling_2012], I then symmetrized the pairwise relationships within
...
computed by the Pairs software described in @gotelli_empirical_2009 as my
null model-based estimator of species interactions.
...
negative relationship passed; other methods failed (Appendix). For the null
model (Pairs), I calculated statistical significance using its $Z$-score. For
the Markov network, I used the Hessian matrix to generate approximate confidence
...
hand, each correctly identified the direct negative interaction between the
shrubs 94% of the time (although the confidence intervals overlapped zero in
most of the replicates).
...
analyses in this field [@gotelli_empirical_2009].
