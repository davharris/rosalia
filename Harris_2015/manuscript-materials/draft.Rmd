...
other ecological processes such as dispersal limitation and environmental filtering
...
While competition between the two shrubs in the previous example does not
...
function*, $Z(\alpha, \beta)$, which is defined so that that the probabilities of all
...
required for exact estimation spirals exponentially into the billions and beyond.
...
used several other techniques for determining the sign and strength of marginal
associations between pairs of species (Appendix B).

The first two alternative interaction measures were the sample covariances and the
...
The third alternative method, described in @gotelli_empirical_2009, involved simulating
...
negative relationship passed; other methods failed.
...
interpretation. In each method, I regressed the "true" $\beta$ coefficient for
each species pair against the model's estimate, re-weighting the pairs so that
each landscape contributed equally to the rescaled estimate. These regressions
yielded squared errors for each of the 23,140 simulated species pairs, across
all conditions and replicates. I then partitioned these errors into groups
defined by the properties of the simulated landscapes (i.e. species richness,
number of observed communities, and the presence/absence of environmental
filtering). Within each partition, the mean squared error was used to
calculate the proportion of variance explained (compared with a baseline model
