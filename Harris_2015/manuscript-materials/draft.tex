...
gradients; the package passes these functions to the ``BFGS'' method in
R's general-purpose optimizer, which then estimates the Markov network
parameters.
...
their average as an overall measure of their relationship.
...
and heterogeneous landscapes, respectively---far higher than the nominal
...
three simulation types. For the null model (Pairs), these values include
the removal of two outliers with \(|Z|>1000\) {[}{[}Appendix{]}{]}.
