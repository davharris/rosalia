...
approximations, based on controlling for indirect effects with partial
...
presented here, I used the rosalia package (Harris 2015a) for the R
programming language (R Core Team 2015) to calculate
\(p(\vec{y}; \alpha, \beta)\) and its gradients exactly (Murphy 2012);
the package passes these functions to the ``BFGS'' method in R's
general-purpose optimizer, which finds values for \(\alpha\) and
\(\beta\).
...
(instead of just their presence/absence); here, interactions between
species occurred on a per-capita basis. To prevent runaway mutualisms
leading to infinite abundance, all interaction coefficients were
negative in these simulations. Good performance on these landscapes
would indicate some robustness to the mechanistic details of species
interactions.
...
network's errors were also more concentrated around 0, so it never
