...
> I now report approximate Type I error rates and the coverage of the model's 95% confidence intervals.  The 95% confidence intervals have about 97.5% coverage when the data was generated from an unmodified Markov network, and about 86% coverage in the presence of environmental heterogeneity [[lines and Appendix]]. These coverage values aren't perfect, but they are reasonable, and they provide far better inferences than ecologists' current methods [[Figure XX]].
...
> Showing the results' dependence on the value of beta is a great idea, and I have added several analyses along these lines in Figure 4 and Appendix 5.  In particular, I now show how CI coverage and the probability that the model will predict the wrong sign for an interaction vary as a function of beta (in addition to global estimates of R-squared and Type I error rates).  If you think that readers would benefit from even more disaggregation, I could include a data file containing the seven models' estimates for all the species pairs across all 450 simulated landscapes as an Appendix.
...
> I have changed the wording to emphasize hypothesis testing rather than assumptions (lines [[XX]]).
...
>This line [[XX]] now describes "how... conditional relationships can determine species composition" rather than referring to "groups".
...
>The discussion of computational difficulties has been removed to save space, and the lines you noted are now at the end of a paragraph rather than the beginning of one (lines [[xx]]).
...
> As described above, I now estimate the coverage of the confidence intervals, and (in the Appendix) show how coverage varies as a function of the "true" parameter value.
...
> I wasn't entirely sure whether you were referring to the logistic regression models or to the process of fitting the Markov network, but I believe that I've clarified the discussion of both issues sufficiently that supplemental material is no longer required for either.  With regard to the logistic regressions, I've now clarified the symmetry issue: the reader should now be able to tell that I am merely averaging the estimated regression coefficient for y_i versus y_j with the coefficient from the regression of y_j on y_i [[lines]].  With regard to the
