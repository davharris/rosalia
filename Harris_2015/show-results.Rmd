...
Exclude the "abundance" simulations because the interpretation of $\beta$ is different.

```{r, cache = TRUE}
rmses = markov_summary %>% filter(simulation_type != "abund") %>% mutate(rmse = sqrt((truth - estimate)^2))

ggplot(rmses, aes(x = truth, y = rmse)) + geom_point(alpha = .2, size = 1) + geom_smooth() + theme_bw() + coord_cartesian(ylim = c(0, max(rmses$rmse)))
```
