...
#### Calculate average R-squared across methods and simulation types
...
error_smoother = function(data, f){
  predict(
    gam(
      f(data) ~ s(truth),
      data = data,
...
p_right_pairs = error_smoother(pairs_summary, confidently_right)
p_wrong_pairs = error_smoother(pairs_summary, confidently_wrong)

p_right_markov = error_smoother(markov_summary, confidently_right)
p_wrong_markov = error_smoother(markov_summary, confidently_wrong)


par(mfrow = c(1, 2))
plot(truth_seq, p_right_pairs + p_wrong_pairs, type = "l", ylim = c(0, 1), yaxs = "i")
lines(truth_seq, p_right_markov + p_wrong_markov, col = 2)
abline(h = 0.05)


plot(truth_seq, p_wrong_pairs, type = "l", ylim = c(0, 1), yaxs = "i")
lines(truth_seq, p_wrong_markov, col = 2)
par(mfrow = c(1, 1))
