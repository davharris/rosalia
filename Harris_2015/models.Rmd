...
How often does each method get the sign of the interaction correct?
```{r, cache = TRUE}
sapply(
  list(partial = partial_results, pairs = pairs_results, rosalia = rosalia_results, cov = cov_results), 
  function(x){
    signif(100 * mean(sign(x$predicted) == sign(x$truth)), 3)
  }
)
```

Compare with the percent correct one would obtain "by chance" if one knew that 75%
of the true interactions were negative
```{r, cache = TRUE}
signif(
   100 *mean(rbinom(10000, size = 1, prob = .75) == rbinom(10000, size = 1, prob = .75)),
  3
)
```


What percentage of absolute $Z$ scores exceed 5?

```{r}
mean(abs(pairs_results$predicted) > 5)
```

What percentage of absolute $Z$ scores produce p-values below `.Machine$double.eps`?

```{r}
.Machine$double.eps

p_values = pnorm(1 - abs(pairs_results$predicted))
mean(p_values < .Machine$double.eps)
```
