...
evaluation = sapply(
  ...
  function(i){
    ...
    c(
      ...
      rosalia = rosalia(x, trace = 0, prior = make_flat_prior())$beta[2,3] < 0
    )
  })
...
# the column identified above, and find its sign
pairs_evaluation = Pairs.txt[grep("(Var2.*Var1)|(Var1.*Var2)", Pairs.txt)] %>% 
    strsplit(" +") %>%
    sapply(. %>% extract(z_score_column) %>% as.numeric) %>%
    is_greater_than(0) # Z>0 implies negative interactions because the Z-score
                       # relates to the distribution of C-scores, which describe
                       # the frequency with which species *do not* co-occur
```
