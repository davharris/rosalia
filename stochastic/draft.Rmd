...
for(i in 1:n_gibbs){
  ...
  beta_grad = (delta_stats - 10 * beta) / 10 / n_loc # Subtract beta as weight decay
...
```{r}
pdf("poster-beta.pdf", width = 4, height = 4)
plot(beta[upper.tri(beta)], true_beta[upper.tri(beta)],
     pch = 16, cex = .4, col = "#00000020",
     xlab = "Estimated interaction strength",
     ylab = "\"True\" interaction strength",
     main = "Recoverd pairwise interactions",
     las = 1,
     cex.axis = 0.75
    )
text(
...
```{r}
pdf("poster-alpha.pdf", width = 4, height = 4)
plot(alpha_env, true_alpha_env,
     ...
     main = "Recovered environmental coefficients",
     las = 1,
     cex.axis = 0.75
)
text(
